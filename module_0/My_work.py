...
limit_number = int(input('До какого числа можно загадать компьютеру?: '))  # нужно для определения возможного интервала
...
def game_core_v3(number):
    ...
    global limit_number
    max_number = limit_number  # какое максимальное чисто мог загадать компьютер
    ...
    while predict != number:
        if predict > number:
            count += 1
            max_number = predict
            predict = int((max_number - min_number) / 2) + min_number  # новое предполагаемое число
        else:
            count += 1
            min_number = predict
            predict = int((max_number + 1 - min_number) / 2) + min_number  # новое предполагаемое число
    return count
